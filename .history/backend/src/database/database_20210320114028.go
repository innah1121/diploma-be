...
func (model *DBModel) GetUserByUsernamePassword(username string,password string) (string, error) {
	var user User
	err := model.db.QueryRow("SELECT * FROM users where username = ? AND password=?", username, password).Scan(&user)
	if err != nil {
		return "", err
	}
