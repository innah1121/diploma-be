...
func handleRequests() {
	...
	myRouter.HandleFunc("/recieveFile", shareFile)
...
func recieveFile(w http.ResponseWriter, r *http.Request) {
	...
	recipient, _ := function.GetUser(recipientUsr, recipientUsr)
	...
	error := user2.ReceiveFile(p.Filename, p.SenderUsr, magic_string)
